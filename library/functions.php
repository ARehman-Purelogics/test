...
function insert_a_bid($data)
{
	$sql = "insert into tbl_bids(auction_id , name, phone, email, price) values(".$data['auction_id'].",'".$data['name']."','".$data['phone']."','".$data['email']."',".$data['price'].")";
	$result = dbQuery($sql);
	return dbInsertId();
	}
...
{
	$sql = "select * from tbl_bids where email='".$email."' && auction_id=".$id;
	$result = dbQuery($sql);
